import numpy as np
import pandas as pd
import rpy2.robjects as robjects  # noqa
...
slingshot.slingshot = SignatureTranslatedFunction(
    slingshot.slingshot,
    init_prm_translate={"start_clus": "start.clus", "end_clus": "end.clus"},
)


# Function to run slingshot on preprocessed data
def run_slingshot(
    adata, terminal_states, root_states, cluster_key="cluster_key", embedding="X_umap"
):
    sling_out = slingshot.slingshot(
        ...
        start_clus=root_states,
        end_clus=terminal_states,
    )
    ...
    for i in range(np.array(slingshot.slingCurves(sling_out)).shape[0]):
        sling_curves.append(
            np.array(np.array(slingshot.slingCurves(sling_out))[i][0])[
    ...
    slingshot_pst_data = pd.DataFrame(
        sling_pst, columns=[sling_lineages[i][-1] for i in range(sling_pst.shape[1])]
    )
    slingshot_pst_data.index.name = "Cell"

    slingshot_alignment_data = pd.DataFrame(
        sling_weights,
