...
sceasy.convertFormat = SignatureTranslatedFunction(
    sceasy.convertFormat, init_prm_translate={"from_format": "from"}
)
...
def run_monocle3(
    ...
    monocleObj = monocle3.order_cells(
        monocleObj,
    ...
    monocle_graph_coordinates = (
        principal_graph_aux.slots["listData"].rx2["UMAP"].rx2["dp_mst"]
    )

    monocle_graph_coordinates = pd.DataFrame(
        monocle_graph_coordinates.T, columns=["coordinate_1", "coordinate_2"]
    )
